...
class Compute {
  ...
  compute_template: string | undefined;
  custom_method: (values: any[]) => any | undefined;
  ...
  _computeCustomMethod(values: any[]) {
  ...
  _computeHighestValue(values: number[]) {
    if (_.isEmpty(values)) return;

    this.attribute[this.output_property] = _.max(values);
  }

  _computeMostFrequentValue(values: any[]) {
    if (_.isEmpty(values)) return;
